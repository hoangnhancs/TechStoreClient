...
import { useCompletePaymentMutation, useCreatePaymentIntentMutation, useCreatePaymentMutation } from "../../features/payment/paymentApi";
import { useState } from "react";
import { Address, Basket, PaymentInfor } from "../../lib/types";
import { setBasketStates } from "../../features/basket/basketSlice";
import { toast } from "react-toastify";
import { useRemovePermanentlyBasketItemsMutation } from "../../features/basket/basketApi";
...
export const useOrderProcessing = () => {
    ...
    const [removePermanentlyBasketItems] = useRemovePermanentlyBasketItemsMutation();
    ...
    const handlePaymentOrder = async () => {
        try {
            if (currentPaymentInfor.paymentMethod === "CreditCard" && currentPaymentInfor.isValid) 
            {
            ...
            else
            {
                try {
                    ...
                    await createOrUpdateOrder({
                        ...
                        orderStatus: "Completed",
                        paymentStatus: "Paid",
                        paymentMethod: currentPaymentInfor.paymentMethod == "wallet" ? currentPaymentInfor.walletType : currentPaymentInfor.paymentMethod,
                    });         
                    const productIds = selectedItems.map((item) => item.productId);
                    const newBasket = await removePermanentlyBasketItems({productIds: productIds});
                    dispatch(setBasketStates({ selectedItems: [], basket: newBasket.data ?? ({ id: "", userId: "", items: [] } as Basket) }));         
                    navigate("/order-success");
                } catch (error) {
                    toast.error("Không thể tạo đơn hàng. Vui lòng thử lại." + error);
                }    
